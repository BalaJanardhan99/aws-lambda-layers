...
class ServerlessLayers {
  ...
  async main() {
    ...
    const hasSettingsChanged = await this.hasSettingsChanged(); // check if directories content has changed
    ...
    const notChanged = !hasFoldersChanged && !hasDepsChanged && !hasSettingsChanged; // merge package default options
  ...
  getLayerName() {
    ...
    const settings = this.getSettings();
    const name = settings[this.currentLayerName] ? settings[this.currentLayerName].name : undefined;
    return slugify(name || `${stackName}-${runtimeDir}-${this.currentLayerName}`, {
      lower: true,
      replacement: '-'
    });
  }
